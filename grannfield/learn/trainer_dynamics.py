                ...
                for i in range(skip_steps, len(self.train_loader)):
                    if not self.stop:
                        ...
                        for k, v in train_batch.items():
                            train_batch = {k: v.to(self.device) for k, v in train_batch.items() if k != 'atom_idx'}
                            train_batch['atom_idx'] = [idx.to(self.device) for idx in v]
                        ...
                        losses.update(loss.data.cpu() / scale, len(train_batch['atom_idx']))
                    ...
                    if self.step % self.validation_interval == 0 or self.stop:
                        ...
                        for i, val_batch in enumerate(self.validation_loader):
                            ...
                            for k, v in val_batch.items():
                                val_batch = {k: v.to(self.device) for k, v in val_batch.items() if k != 'atom_idx'}
                                val_batch['atom_idx'] = [idx.to(self.device) for idx in v]
                            ...
                            val_losses.update(val_batch_loss, len(val_batch['atom_idx']))
                        ...
                        if self.best_loss > val_losses.avg:
                            ...
                            try:
                                ...
                                print("Caution: Model's state_dict was saved instead of the whole model.")
    ...
    def test(self, test_loader):
        ...
        for test_batch in test_loader:
            ...
            for k, v in test_batch.items():
                test_batch = {k: v.to(self.device) for k, v in test_batch.items() if k != 'atom_idx'}
                test_batch['atom_idx'] = [idx.to(self.device) for idx in v]
