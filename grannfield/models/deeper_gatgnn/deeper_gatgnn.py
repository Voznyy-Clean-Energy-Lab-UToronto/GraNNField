...
from torch_geometric.utils import softmax as tg_softmax, degree
...
class GATGNN_GIM1_globalATTENTION(torch.nn.Module):
    ...
    def forward(self, x, batch, glbl_x=None):
        if glbl_x is not None:
...
class DEEP_GATGNN(torch.nn.Module):
    def __init__(
        ...
        self.expansion = GaussianSmearing(0, self.cutoff, self.num_edge_features)
        ...
        if pre_fc_count == 0:
            gc_dim = self.num_features
        else:
            gc_dim = dim1
        ##Determine post_fc dimension
        if pre_fc_count == 0:
            post_fc_dim = self.num_features
        else:
            post_fc_dim = dim1
        ...
        if pre_fc_count > 0:
            ...
            for i in range(pre_fc_count):
                if i == 0:
                    lin_N = torch.nn.Linear(self.num_features, dim1)
                    self.pre_lin_list_N.append(lin_N)
                    lin_E = torch.nn.Linear(self.num_edge_features, dim1)
    ...
    def forward(self, data):
        ...
        for i in range(0, len(self.conv_list)):
            if len(self.pre_lin_list_N) == 0 and i == 0:
                if self.batch_norm == "True":
                    out_x = self.conv_list[i](x, edge_index, edge_attr)
                    out_x = self.bn_list[i](out_x)
                else:
                    out_x = self.conv_list[i](x, edge_index, edge_attr)
            else:
                if self.batch_norm == "True":
                    out_x = self.conv_list[i](out_x, edge_index, out_e)
                    out_x = self.bn_list[i](out_x)
                else:
                    out_x = self.conv_list[i](out_x, edge_index, out_e)
        ...
        # print(out_x.shape)
        # exit()
        out_a = self.global_att_LAYER(out_x, data['batch'])
        out_x = (out_x) * out_a

        ##Post-GNN dense layers
        if self.pool_order == "early":
            if self.pool == "set2set":
                if self.glbl_x is not None:
                    out_x = self.set2set(out_x, data['batch'], self.glbl_x)
                else:
