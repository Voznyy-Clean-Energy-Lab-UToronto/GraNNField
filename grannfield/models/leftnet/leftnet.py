...
yuanqidu/LeftNet: https://github.com/yuanqidu/LeftNet
"""
...
from grannfield.utils.utils import radius_graph_pbc
...
class LEFTNet(torch.nn.Module):
    ...
    def forward(self, data):
        if self.use_pbc:
            ...
            i, j = edge_index
            edge_diff = data['positions'][i] - data['positions'][j]

            # correct for pbc
            neighbors = neighbors.to(data['cell'].device)
            cell = torch.repeat_interleave(data['cell'], neighbors, dim=0)
            offsets = cell_offsets.float().view(-1, 1, 3).bmm(cell.float()).view(-1, 3)
            edge_diff += offsets

            # compute distances
            dist = edge_diff.norm(dim=-1)

            # redundancy: remove zero distances
            nonzero_idx = torch.arange(len(dist)).to(cell.device)
            nonzero_idx = nonzero_idx[dist != 0]
            edge_index = edge_index[:, nonzero_idx]
            dist = dist[nonzero_idx]

            edge_diff = edge_diff[nonzero_idx]
        else:
            ...
            dist = (data['positions'][i] - data['positions'][j]).norm(dim=-1)
            edge_diff = data['positions'][i] - data['positions'][j]

        # embed z
        z_emb = self.z_emb(data['atomic_numbers'])
        ...
        s = self.neighbor_emb(data['atomic_numbers'], z_emb, edge_index, radial_hidden)
        ...
        edge_cross = torch.cross(data['positions'][i], data['positions'][j])
        ...
        mean_neighbor_pos = self.mean_neighbor_pos(data['positions'], edge_index)
        node_diff = data['positions'] - mean_neighbor_pos
        node_diff = _normalize(node_diff)
        node_cross = torch.cross(data['positions'], mean_neighbor_pos)
