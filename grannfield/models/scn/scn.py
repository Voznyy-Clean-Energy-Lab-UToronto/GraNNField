...
from grannfield.models.scn.sampling import CalcSpherePoints
from grannfield.models.scn.smearing import (
    GaussianSmearing,
    LinearSigmoidSmearing,
    SigmoidSmearing,
    SiLUSmearing,
)
from grannfield.models.scn.spherical_harmonics import SphericalHarmonicsHelper
from grannfield.utils.utils import get_max_neighbors_mask, radius_graph_pbc, get_pbc_distances
...
class SphericalChannelNetwork(nn.Module):
    ...
    def __init__(
        ...
        if num_resolutions == 2:
            ...
            self.cutoff_list = torch.tensor(
                [self.cutoff - 0.01, self.max_num_neighbors - 0.01]
            )
    ...
    def forward(self, data):
        ...
        self.num_atoms = len(data['atomic_numbers'])
    ...
    def _forward_helper(self, data):
        ...
        edge_index, cell_offsets, _, neighbors = radius_graph_pbc(
            data, self.cutoff, self.max_num_neighbors, self.offset_degree
        )
        ...
        energy = torch.zeros(len(data['n_atoms']), device=data['positions'].device)
        ...
        if torch.min(edge_vec_0_distance) < 0.0001:
            ...
            print(
                "Error edge_vec_0_distance: {} {} {} {} {}".format(
                    ...
                    edge_index[0, minidx],
                    edge_index[1, minidx],
                    data['positions'][edge_index[0, minidx]],
                    data['positions'][edge_index[1, minidx]],
                )
            )
